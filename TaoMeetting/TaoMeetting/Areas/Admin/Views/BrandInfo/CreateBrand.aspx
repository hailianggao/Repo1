     ...
     <%using (Html.BeginForm()){ %>
       <table>
         <tr><td>品牌名称</td><td><%=Html.TextBoxFor(bi=>bi.BrandName) %></td></tr>
          <tr><td colspan="2">
         <img src="" id="image" name="image" style="display:none" />
         </td></tr>
         <tr><td>上传图片</td><td>
        ...
        <input type="file" name="uploadify" id="uploadify" />
         ...
         <tr><td>地址</td><td><%=Html.DropDownList("city",ViewData["city"] as SelectList,"--请选择地址--",new{}) %></td></tr>
          ...
          <tr><td>基本功能:</td><td>
             ...
             <%
               ...
               %>
               <tr><td align="right">
               <div><span><%=ft.FunName%><input type="checkbox" id="BaseFuns" name="BaseFuns" value="<%=ft.id %>"/></span></div>
               </td></tr>
               <%
                }
                  %>
          ...
          <%} if (ViewData["fts"] != null)
           ...
           <table>
             <%
               ...
               %>
               <tr><td align="right">
               <div><span><%=ft.FunName%><input type="checkbox" id="SpecFuns" name="SpecFuns" value="<%=ft.id %>"/></span></div>
               </td></tr>
               <%
                }
                  %>
              ...
              { %>
               <tr> <td>产品用途:</td><td>
                  <%foreach (ProductType pt in ViewData["product"] as IList<ProductType>)
                    {
                        %>
              <div><span style="text-align:right"><%=pt.TypeName%><input type="checkbox" id="BrandProducts" name="BrandProducts" value="<%=pt.id%>"/></span></div>
                        <%
                    } %>
               </td></tr>
            <%} %>
                ...
                <%foreach (BuyType bt in ViewData["buytype"] as IList<BuyType>)
                  {
                    %>
                    <div><%=bt.BuyTypeName %>
             <input type="checkbox" id="ckbt<%=bt.id%>" name="ckbt<%=bt.id%>"  onclick="ShowPriceDiv(<%=bt.id %>)" value='<%=bt.id %>'/><span id="pricediv<%=bt.id %>" style="display:none; width:120px;"><input type="text" name="txtprice<%=bt.id%>" id="txtprice<%=bt.id%>"  onchange="SetPrice(<%=bt.id%>,this)" style="width:50px;" />元/点/月</span></div>
                    <%
                  } %>
                </td></tr>
             <%} %>
